...
export class ResourcesService {
  ...
  private static mapGetResourcesResponse(response: any): Resource[] {
    ...
    return response.map(responseItem => {
      const date = new Date(responseItem.timestamp);
      const resource = new Resource();
      resource.timestamp = `${twoDigitValue(date.getHours())}:${twoDigitValue(date.getMinutes())}:${twoDigitValue(date.getSeconds())}:${twoDigitValue(date.getMilliseconds())}`;
